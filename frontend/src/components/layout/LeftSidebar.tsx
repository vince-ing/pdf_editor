...
// Hamburger and lightbulb moved to TopBar. Rail shows view icons only.

import {
  FileText, Bookmark, MessageSquare,
  Layers, Search, ChevronRight, RotateCw, GripVertical,
} from 'lucide-react';
...
const PlaceholderView = ({ icon: Icon, title, hint }: {
...
export function LeftSidebar({
  ...
  return (
    <div className="h-full bg-[#25292d] border-r border-[#1e2327] flex flex-shrink-0">
      ...
      {/* ── Icon rail — same dark color as TopBar ── */}
      <div className="w-12 bg-[#1e2327] flex flex-col items-center py-3 gap-1 flex-shrink-0">
        {VIEW_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-10 h-10 flex items-center justify-center rounded transition-colors
                ${isActive ? 'text-[#4a90e2] bg-[#2d3338]' : 'text-gray-500 hover:text-white hover:bg-[#2d3338]'}`}
            >
              <Icon size={18} />
            </button>
          );
      ...
      {drawerView && (
        <div className="w-44 flex flex-col animate-slide-right overflow-hidden">
          ...
          <div ref={scrollContainerRef} className="flex-1 overflow-y-auto py-2 px-3 space-y-2">
                ...
                {!pdfDoc && pages.length === 0 && (
                  [1, 2, 3, 4].map(n => (
                    <div key={n} className={`relative cursor-default rounded-lg overflow-hidden ${n === 1 ? 'ring-2 ring-[#4a90e2]' : ''}`}>
                      <div className="aspect-[8.5/11] bg-[#2d3338] rounded-lg">
                      ...
                      <div className="absolute bottom-1 right-1.5 bg-[#1e2327] text-gray-500 text-[9px] px-1.5 py-0.5 rounded font-mono">{n}</div>
                    </div>
                  ))
                )}
            ...
            {drawerView === 'bookmarks'   && <PlaceholderView icon={Bookmark}      title="No bookmarks yet"  hint="Use Insert › Bookmark to add one." />}
            {drawerView === 'annotations' && <PlaceholderView icon={MessageSquare} title="Annotations"       hint="Annotations list coming soon." />}
            {drawerView === 'layers'      && <PlaceholderView icon={Layers}        title="Layers"            hint="Layers panel coming soon." />}
