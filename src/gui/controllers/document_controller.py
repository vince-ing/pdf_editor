...
class DocumentController:
    """Manages file I/O operations, image-to-PDF staging, and document state."""
    def __init__(
        self, root: tk.Tk, image_conversion_service: Any, merge_split_service: Any,
        history: Any, recent_files: Any, viewport: Any, tool_manager: Any, ui: dict[str, Callable]
    ) -> None:
        self.root = root
        self.image_conversion_service = image_conversion_service
        self.merge_split_service = merge_split_service
        self.history = history
        self.recent = recent_files
    ...
    def update_title(self) -> None:
        if self.current_path:
            ...
            marker = " •" if self.unsaved_changes else ""
            title = f"{name}{marker}"
        else:
            title = "PDF Editor" + (" — Untitled •" if self.unsaved_changes else "")
        
        self.root.title(f"PDF Editor — {title}" if self.current_path else title)
        
        top_bar_title = os.path.basename(self.current_path) + (" •" if self.unsaved_changes else "") if self.current_path else "PDF Editor"
        self.ui["set_top_bar_title"](top_bar_title)
    ...
    def get_image_thumbnail(self, path: str, width: int) -> bytes:
    ...
    def open_merge_split_dialog(self) -> None:
        if _HAS_MERGE_SPLIT:
            MergeSplitDialog(root=self.root, service=self.merge_split_service, current_doc=self.doc, on_open_path=self.open_pdf_path)