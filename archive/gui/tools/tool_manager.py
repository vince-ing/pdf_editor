...
class ToolManager:
    def __init__(self, ctx: AppContext, services: dict[str, Any], ui_callbacks: dict[str, Callable], settings: Any) -> None:
        self.ctx = ctx
        self.services = services
        self.ui = ui_callbacks
        self.settings = settings
        
        # Load style preferences directly from SettingsManager
        self.style = self.settings.get("tool_styles")
    ...
    def _init_tools(self) -> None:
        s = self.style
        srv = self.services
        self.tools["highlight"] = AnnotationTool(self.ctx, srv["annotation"], "highlight", get_stroke_rgb=lambda: s["annot_stroke_rgb"], get_fill_rgb=lambda: s["annot_fill_rgb"], get_width=lambda: s["annot_width"])
        self.tools["rect_annot"] = AnnotationTool(self.ctx, srv["annotation"], "rect_annot", get_stroke_rgb=lambda: s["annot_stroke_rgb"], get_fill_rgb=lambda: s["annot_fill_rgb"], get_width=lambda: s["annot_width"])
        ...
        self.tools["redact"]       = RedactTool(self.ctx, srv["redaction"], get_fill_color=lambda: s["redact_fill_color"], get_replacement_text=lambda: s["redact_label"], on_navigate_page=self.ui["navigate_to"], on_hit_changed=self._on_search_hit_changed)
        self.tools["draw"]         = DrawTool(self.ctx, annotation_service=srv["annotation"], get_mode=lambda: s["draw_mode"], get_stroke_rgb=lambda: s["draw_stroke_rgb"], get_fill_rgb=lambda: s["draw_fill_rgb"], get_width=lambda: s["draw_width"], get_opacity=lambda: s["draw_opacity"], on_committed=self._on_draw_committed)

    def get_tool(self, name: str): return self.tools.get(name)
    ...
    def select_tool(self, name: str) -> None:
        ...
        if name != "redact" and self.get_tool("redact") and self.get_tool("redact").has_pending_hits: self.get_tool("redact").cancel_hits()

        cursor_map = {"text": "crosshair", "insert_image": "crosshair", "highlight": "crosshair", "rect_annot": "crosshair", "select_text": "ibeam", "extract": "arrow", "redact": "crosshair", "draw": "crosshair", "edit_text": "ibeam"}
    ...
    def on_tool_style_change(self, key: str, value) -> None:
        # CRITICAL FIX: Actually save the new style into the dictionary!
        if key not in ("redact.find", "redact.confirm", "redact.cancel"):
            self.style[key] = value
            self.settings.set("tool_styles", self.style)

        if key == "redact.find": self._redact_find_from_props(value)
        elif key == "redact.confirm": self._redact_confirm()
        elif key == "redact.cancel": self._redact_cancel_hits()
        
        if key in ("font_index", "fontsize", "text_color", "text_align"):
            if self.text_boxes and self.active_tool_name == "text":
                self.text_boxes[-1].update_style(font_index=self.style["font_index"], fontsize=self.style["fontsize"], color_rgb=self.style["text_color"], align=self.style["text_align"])

    def on_tool_state_change(self, key: str, value) -> None: pass
    ...
    def handle_click(self, cx: float, cy: float) -> None:
        ...
        elif self.current_tool: self.current_tool.on_click(cx, cy)

    def handle_drag(self, cx: float, cy: float) -> None:
        if self.current_tool: self.current_tool.on_drag(cx, cy)

    def handle_release(self, cx: float, cy: float) -> None:
        if self.current_tool: self.current_tool.on_release(cx, cy)

    def handle_motion(self, cx: float, cy: float) -> None:
        if self.active_tool_name == "select_text" and self.current_tool: self.current_tool.on_motion(cx, cy)
    ...
    def _spawn_textbox(self, pdf_x: float, pdf_y: float) -> None:
        ...
        pdf_w, pdf_h, bg = page.width * 0.42, self.style["fontsize"] * 4, self._sample_page_color(pdf_x, pdf_y)
        box = TextBox(canvas=self.ctx.canvas, pdf_x=pdf_x, pdf_y=pdf_y, pdf_w=pdf_w, pdf_h=pdf_h, scale=self.ctx.scale, page_offset_x=self.ctx.page_offset_x, page_offset_y=self.ctx.page_offset_y, font_index=self.style["font_index"], fontsize=self.style["fontsize"], color_rgb=self.style["text_color"], entry_bg=bg, align=self.style["text_align"], on_commit=self._on_box_confirmed, on_delete=self._on_box_deleted, on_interact=self._on_box_interact)
    ...
    def _sample_page_color(self, pdf_x: float, pdf_y: float) -> str:
        ...
        try: return f"#{img.get(int(pdf_x * self.ctx.scale), int(pdf_y * self.ctx.scale))[0]:02x}{img.get(int(pdf_x * self.ctx.scale), int(pdf_y * self.ctx.scale))[1]:02x}{img.get(int(pdf_x * self.ctx.scale), int(pdf_y * self.ctx.scale))[2]:02x}"
    ...
    def _on_box_confirmed(self, box: TextBox) -> None:
        ...
        try:
            ...
            cmd.cleanup()
            return messagebox.showerror("Insert Error", str(ex))
        self.ctx.render()

    def _on_box_interact(self) -> None: self.suppress_next_click = True
    def _on_box_deleted(self, box: TextBox) -> None: self.text_boxes = [b for b in self.text_boxes if b is not box]
    ...
    def rescale_boxes(self) -> None:
        for box in list(self.text_boxes): box.rescale(self.ctx.scale, self.ctx.page_offset_x, self.ctx.page_offset_y)
    def copy_selected_text(self) -> None:
        if t := self.get_tool("select_text"): t.copy()

    def _on_search_hit_changed(self, cur_idx: int, total: int) -> None:
        if total == 0 or cur_idx < 0: return self.ui["update_hit_display"](-1, 0)
    ...
    def _redact_find_from_props(self, params: dict) -> None:
        ...
        if not query: return self.ctx.flash_status("Enter a search term first", color=PALETTE["fg_secondary"])
    ...
    def _redact_confirm(self) -> None:
        if rt := self.get_tool("redact"): rt.redact_all_hits()
        self.ui["hide_redact_confirm"]()
        self.ui["clear_hit_display"]()

    def _redact_cancel_hits(self) -> None:
        if rt := self.get_tool("redact"): rt.cancel_search()
        self.ui["hide_redact_confirm"]()
        self.ui["clear_hit_display"]()
    ...
    def _on_draw_committed(self, page_idx: int, xref: int) -> None:
        ...
        if viewport := getattr(self.ctx._editor, "viewport", None):
